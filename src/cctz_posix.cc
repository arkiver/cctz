...
namespace cctz {
  ...
  namespace {
    ...
    // datetime = Mm.w.d [ / [+|-]hh[:mm[:ss]] ]
    const char* ParseDateTime(const char* p, PosixTransition* res) {
      // We only handle 'M' rules as they are the only ones generated by zic.
      if (p != nullptr && *p == ',' && *++p == 'M') {
        int month = 0;
        if ((p = ParseInt(p + 1, 1, 12, &month)) != nullptr && *p == '.') {
          ...
          if ((p = ParseInt(p + 1, 1, 5, &week)) != nullptr && *p == '.') {
            ...
            if ((p = ParseInt(p + 1, 0, 6, &weekday)) != nullptr) {
              ...
              res->week = week;
              res->weekday = weekday;
              res->offset = 2 * 60 * 60;  // default offset is 02:00:00
              if (*p != '/') return p;
              p = ParseOffset(p + 1, -167, 167, 1, &res->offset);
